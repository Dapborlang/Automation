...
class FormBuilderController extends Controller
{
    ...
    public function create($id)
    {
        ...
        if(sizeof((array)$foreign)>0)
        {
            foreach (array_keys($foreign) as $key) {
                ...
                if ($tableName->getConnection()
                ->getSchemaBuilder()
                ->hasColumn($tableName->getTable(), 'finalize')) 
                {
                    $data=$values::where('finalize',false)->get();
                }
                else
                {
                    $data=$values::all();
    ...
    public function update(Request $request, $id,$cid)
    {
        ...
        if(isset($request->redirect) && $request->redirect!='')
        {
            return redirect($request->redirect)->with('message', 'Updated Successfully');
        }
        else
        {
    ...
    {        
        ...
        $exclude=json_decode($model->index->exclude);
        $columns = \DB::connection()->getSchemaBuilder()->getColumnListing($model->table_name);
        $table=$table->where($_GET['column'],$cid)->get();
        $select=array();
        if(sizeof((array)$masterKey)>0)
        {
            foreach (array_keys($masterKey) as $item) {
        ...
        return view($model->route.'.ajax', compact('columns','model','exclude','table','select','master'));
    }
